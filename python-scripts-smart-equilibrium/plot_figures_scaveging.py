...
def plot_figures_calcite_dolomite():
    for i in plot_at_selected_steps:
        ...
        plt.plot(xcells[status[i - 1] == 0], data_smart_calcite[status[i - 1] == 0] * 100 / (1 - phi), 'o',
                 **line_empty_marker('C0'))
        plt.plot(xcells[status[i - 1] == 1], data_smart_calcite[status[i - 1] == 1] * 100 / (1 - phi), 'o',
                 **line_filled_marker('C0'))
        plt.plot(xcells[status[i - 1] == 0], data_smart_dolomite[status[i - 1] == 0] * 100 / (1 - phi), 'o',
                 **line_empty_marker('C1'))
        plt.plot(xcells[status[i - 1] == 1], data_smart_dolomite[status[i - 1] == 1] * 100 / (1 - phi), 'o',
                 **line_filled_marker('C1'))
        plt.plot([], [], 'o', label='Smart Prediction', **line_filled_marker('black'))
        plt.plot([], [], 'o', label='Learning', **line_empty_marker('black'))
        plt.legend(loc='center right')
...
def plot_figures_aqueous_species():
    for i in plot_at_selected_steps:
        ...
        plt.plot(xcells[status[i - 1] == 0], data_smart_cacation[status[i - 1] == 0], 'o', **line_empty_marker('C0'))[
            0],
        plt.plot(xcells[status[i - 1] == 1], data_smart_cacation[status[i - 1] == 1], 'o', **line_filled_marker('C0'))[
            0],
        plt.plot(xcells[status[i - 1] == 0], data_smart_mgcation[status[i - 1] == 0], 'o', **line_empty_marker('C1'))[
            0],
        plt.plot(xcells[status[i - 1] == 1], data_smart_mgcation[status[i - 1] == 1], 'o', **line_filled_marker('C1'))[
            0],
        plt.plot(xcells[status[i - 1] == 0], data_smart_hco3anion[status[i - 1] == 0], 'o', **line_empty_marker('C2'))[
            0],
        plt.plot(xcells[status[i - 1] == 1], data_smart_hco3anion[status[i - 1] == 1], 'o', **line_filled_marker('C2'))[
            0],
        ...
        plt.plot(xcells[status[i - 1] == 0], data_smart_hcation[status[i - 1] == 0], 'o',
                 **line_empty_marker('darkviolet'))[0],
        plt.plot(xcells[status[i - 1] == 1], data_smart_hcation[status[i - 1] == 1], 'o',
                 **line_filled_marker('darkviolet'))[0],
        plt.plot([], [], 'o', label='Smart Prediction', **line_filled_marker('black'))
        plt.plot([], [], 'o', label='Learning', **line_empty_marker('black'))
        plt.legend(loc='upper right')
...
def plot_computing_costs():
    ...
    plt.yscale('log')
    plt.ticklabel_format(style='plain', axis='x')
    plt.plot(time_steps[0:nsteps:step], timings_equilibrium_class[0:nsteps:step],
             label="Chemical Equilibrium (Conventional)", color='C0', linewidth=1)
    plt.plot(time_steps[0:nsteps:step], timings_equilibrium_smart[0:nsteps:step], label="Chemical Equilibrium (Smart)",
             color='C1', linewidth=1)
    plt.plot(time_steps[0:nsteps:step], timings_equilibrium_smart_ideal[0:nsteps:step],
             label="Chemical Equilibrium (Smart, Ideal)", color='C3', linewidth=1)
    plt.plot(time_steps[0:nsteps:step], timings_transport[0:nsteps:step], label="Transport", color='C2', linewidth=1)
    leg = plt.legend(loc='lower right', bbox_to_anchor=(1, 0.53))
    for line in leg.get_lines(): line.set_linewidth(2.0)
    plt.tight_layout()
    # plt.show()
...
def plot_on_demand_learnings_total():
    ...
    plt.xlabel('Time Step')
    plt.ylabel('Accumulated on-demand learnings')
...
def plot_on_demand_learnings_total_initial_steps(nsteps_initial):
    ...
    plt.xlabel('Time Step')
    plt.ylabel('Accumulated on-demand learnings (over all steps)')
...
def plot_computing_costs_vs_total_learnings():
    ...
    plt.plot(time_steps[0:nsteps:step], timings_taylor[0:nsteps:step], label="Taylor Extrapolation", color='C1',
             linewidth=1)
...
def plot_speedups():
    ...
    plt.ylabel('Speedup (-)')
    plt.ticklabel_format(style='plain', axis='x')
    plt.plot(time_steps[0:nsteps:step], speedup_ideal[0:nsteps:step],
             label="Speedup: Conventional vs. Smart (Ideal search)", color='C0', linewidth=1)
    plt.plot(time_steps[0:nsteps:step], speedup[0:nsteps:step], label="Speedup: Conventional vs. Smart ", color='C1',
             linewidth=1)
    ...
    try:
        ...
        print("""No permission to save plots to this directory""")
    plt.close()
...
if __name__ == '__main__':
    ...
    files_smart = [file for file in natsorted(os.listdir(folder_smart)) if
                   ("profiling" not in file and "statuses" not in file)]
