...
#plot_at_selected_steps = [1, 10, 60, 120, 480, 960, 1000, 2400, 3600, 5800, 7200]  # the time steps at which the results are plotted
...
folder = 'rt-approx-hessian-nof-with-prop-withcutoff'
folder_class = 'rt-approx-hessian-nof-with-prop-withcutoff'
...
def plot_computing_costs():
    ...
    plt.plot(time_steps[0:nsteps:step], timings_kinetics_class[0:nsteps:step], label="Chemical Kinetics (Conventional)", color='C0', linewidth=2)
    plt.plot(time_steps[0:nsteps:step], timings_kinetics_smart[0:nsteps:step], label="Chemical Kinetics (Smart)", color='C1', linewidth=2, alpha=1.0)
    plt.plot(time_steps[0:nsteps:step], timings_kinetics_smart_ideal[0:nsteps:step], label="Chemical Kinetics (Smart, Ideal)", color='C3', linewidth=2, alpha=1.0)
...
def plot_on_demand_learning_countings():
    ...
    status_learnings = data.get('smart_kinetics_cells_where_learning_was_required_at_step')
...
def plot_computing_costs_of_estimation():
    ...
    plt.yscale('log')
    plt.ticklabel_format(style='plain', axis='x')
    plt.plot(time_steps[0:nsteps:step], timings_search[0:nsteps:step], label="Search", color='C0', linewidth=2)
    plt.plot(time_steps[0:nsteps:step], timings_estimate[0:nsteps:step], label="Estimation", color='C1', linewidth=2)

    leg = plt.legend(loc='lower right')
    for line in leg.get_lines(): line.set_linewidth(2.0)
    plt.tight_layout()
    plt.savefig(folder_general + '/estimate-search-costs.png')
    plt.close()
...
def plot_speedups():
    ...
    plt.xlim(left=0, right=nsteps)
    plt.ticklabel_format(style='plain', axis='x')
    plt.plot(time_steps[0:nsteps:step], speedup_ideal[0:nsteps:step], label="Conventional vs. Smart (Ideal search)", color='C2', linewidth=2, alpha=1.0)
    ...
    plt.savefig(folder_general + '/speedups-nolegend-withpperbound.png')
    plt.close()
